...
export function useJamPage(slug) {
  ...
  useEffect(() => {
    if (songAutocompleteRef.current?.focus) {
  ...
  const handleAddSongToJam = async (songId) => {
    try {
      if (!jam || !jam._id) {
        throw new Error('Jam not loaded yet');
      }
  ...
  const handleFocusExistingSong = (jamSongId) => {
    setScrollToSongId(jamSongId);
  };
  ...
  const handleAddSong = async (newSong) => {
    try {
      ...
      // Check if jam is loaded
      if (!jam || !jam._id) {
        toast.error('Jam not loaded yet');
        setIsAddModalOpen(false);
        return;
      }
