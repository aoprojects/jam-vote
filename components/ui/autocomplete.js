        ...
        <div className="p-1">
          {isLoading ? (
            ...
            (options || []).map((item, index) => {
              const isNonInteractive = item?.isSection || item?.isPlaceholder;
              ...
              if (isNonInteractive) {
                return (
                  <li
                    key={item.value}
                    className={cn(
                      "relative flex w-full select-none items-center rounded-sm px-3 py-2 text-sm text-muted-foreground",
                      item.isSection && "text-xs uppercase tracking-wide font-semibold text-gray-500 cursor-default",
                      item.isPlaceholder && "cursor-default"
                    )}
                  >
                    {renderOption ? renderOption(item, false) : item.label}
                  </li>
                );
              }
